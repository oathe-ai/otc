...
|-----------|--------|-------------|-----------------|
| `prompt_injection` | 0.30 | Attempts to override agent instructions or manipulate LLM behavior | T3.1, T3.2 |
| `data_exfiltration` | 0.25 | Unauthorized transmission of data to external endpoints | T1.1, T1.5 |
| `code_execution` | 0.20 | Unauthorized process execution, filesystem access, or resource abuse | T2.1, T2.2, T5.1, T5.2, T6.1 |
| `clone_behavior` | 0.10 | Behavior that modifies the host system beyond the skill's directory | T4.1 |
| `canary_integrity` | 0.10 | Whether the skill tampers with monitoring canaries | *No pattern-based detection in v1.0.0 — scored by model grader* |
| `behavioral_reasoning` | 0.05 | Overall behavioral assessment by the model grader | *No pattern-based detection in v1.0.0 — scored by model grader* |
